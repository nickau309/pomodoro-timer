...
import { ColorThemeHandle, Slot } from "../../types";
...
export default function Setting() {
  ...
  const [slot, setSlot] = useState("" as Slot);
  const [isColorPickerOpen, setIsColorPickerOpen] = useState(false);
  const themeHandleRef = useRef<ColorThemeHandle | null>(null);

  // focus back to the button of choosing color theme
  // after the panel is switched back to content
  useEffect(() => {
    if (!isColorPickerOpen) {
      themeHandleRef.current?.focus(slot);
    }
  }, [slot, isColorPickerOpen]);

  const openColorPicker = (value: Slot) => {
    setSlot(value);
